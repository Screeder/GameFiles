...
{
	internal class ActionRecords : IActionContainer
	{
        ...
        */
		...
		internal SortedList<uint, uint> LineNumbers = new SortedList<uint, uint>();
		/// <summary>Which lines require labels (as they would be the target of jumps etc).</summary>
		internal List<uint> LabeledLines = new List<uint>();
		private uint lineIndex = 1; // lines start at one

		internal static ConstantPool CurrentConstantPool;
		internal static SortedList<uint, uint> CurrentLineNumbers;
		internal static List<uint> CurrentLabeledLines;

		internal ActionRecords()
		{
		}
        ...
        private void AddScope(DefineFunction scope, SwfReader r)
        {
		...
		internal static void AutoLineLabel(IndentedTextWriter w)
		{
		...
		internal static string GetLabel(int offset)
		{
			string result;
        ...
        internal static IAction CurrentDumpStatement;

	...
	/*
		...
		ZlibBitmapData 		If BitmapFormat = 3			ZLIB compressed bitmap data
							...
							If BitmapFormat = 4 or 5
								ALPHABITMAPDATA
	*/
    internal class DefineBitsLosslessTag : ISwfTag
	{
        internal UInt16 CharacterId;
        internal uint Width;
        internal uint Height;
        internal bool isIndexedColors = false;

        internal uint ColorCount;
        internal RGBA[] ColorTable;
        internal uint[] ColorData;
        internal RGBA[] BitmapData;

        internal BitmapFormat BitmapFormat;
        internal bool HasAlpha;
        internal uint TagLen = 0;

        internal byte[] OrgBitmapData;

        internal DefineBitsLosslessTag(bool hasAlpha)
		...
		{
			Bitmap bmp = new Bitmap((int)this.Width, (int)this.Height, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
		...
		internal override void ToSwf(SwfWriter w)
        {
            ...
            if (BitmapFormat == BitmapFormat.Colormapped8Bit) // 8-bit colormapped image
            {
                ...
                byte[] zipped = ByteReader.Compress(mapData);
